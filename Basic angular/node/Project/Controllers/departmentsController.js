...
exports.getDepartment = (request, response, next) => {
    Department.findOne({ _id: Number(request.params.id) })
        .then(data => {

            if (!data) {
                next(new Error("Department is not found"))
...
exports.updateDepartment = (request, response, next) => {
    ...
    Department.findByIdAndUpdate({_id:request.params.id}, {
        $set: {
            name: request.body.name,
            location: request.body.location
        }
...
exports.deleteDepartment = async (request, response, next) => {
    try {
        let data = await Department.findByIdAndDelete({_id:request.params.id})
        if (data == null) throw new Error("Department Is not Found!")
        response.status(200).json({ message: "deleted" })
    } catch (error) {
        next(error)
